    ...
    class VBox {
        ...
        func getAverage(forceRecalculate force: Bool = false) -> Color {
            if let average = average, !force {
                ...
                let average: Color
                print("[Average] ntot: \(ntot)")
                print("[Average] r sum: \(rSum)")
                print("[Average] g sum: \(gSum)")
                print("[Average] b sum: \(bSum)")
                if ntot > 0 {
...
//        print("make histo pixelCount: \(pixelCount)")
//        var total = 0;
        for i in stride(from: 0, to: pixelCount, by: quality) {
//            print("make Histo stride i: \(i)")
            let a = pixels[i * 4 + 0]
            let b = pixels[i * 4 + 1]
            let g = pixels[i * 4 + 2]
            let r = pixels[i * 4 + 3]
...
//        print("total: ", total)
    ...
    static func quantize(_ pixels: [UInt8], quality: Int, ignoreWhite: Bool, maxColors: Int) -> ColorMap? {
        ...
        for i in 0..<pq.count {
            ...
            print("start--------")
            print("box index: \(i)")
            print("box r range: \(item.rRange)")
            print("box g range: \(item.gRange)")
            print("box b range: \(item.bRange)")
            print("box average: \(item.getAverage())")
            print("end--------")
            colorMap.push(item)
        }
